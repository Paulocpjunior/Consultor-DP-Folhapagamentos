...
export default function ExportacaoIobModal({ modo, onFechar, onModo, quantidade, ocupado, onExportar, onConferencia, onModeloExcel, onCadastroIob }: Props) {
    ...
    return <dialog ref={ref} onCancel={e => { e.preventDefault(); if (!ocupado) onFechar(); }} aria-labelledby="titulo-exportacao-iob" className="w-[min(95vw,640px)] rounded-xl bg-white p-6 text-slate-800 shadow-xl backdrop:bg-black/50 dark:bg-slate-800 dark:text-white">
        ...
        {cadastro && <p className="mb-4 rounded bg-amber-50 p-3 text-sm text-amber-900"><strong>Compatibilidade cadastral IOB pendente de homologação.</strong> O cadastro para a IOB (TXT da rotina Importação de Funcionários, Excel de cadastro e XMLs S-2200) é gerado em <em>Cadastro IOB</em>. O Excel preenchido, o pacote JSON e o CSV são para conferência. O TXT mensal contém apenas matrícula, evento, referência e valor; não transporta nome, CPF ou endereço. As pendências de histórico acompanham os dados.</p>}
        <div className="flex flex-wrap gap-3">
            {cadastro && onCadastroIob && <button disabled={ocupado} onClick={onCadastroIob} className="rounded bg-emerald-700 px-3 py-2 text-sm text-white disabled:opacity-40">Abrir cadastro IOB (TXT · Excel · XML)</button>}
