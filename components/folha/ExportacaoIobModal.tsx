...
interface Props {
    ...
    quantidade: number | null; ocupado?: boolean; onExportar: () => void; onConferencia?: () => void;
}
/** Mesmo diálogo para implantação e rotina mensal; cada modo possui seu próprio contrato de saída. */
export default function ExportacaoIobModal({ modo, onFechar, onModo, quantidade, ocupado, onExportar, onConferencia }: Props) {
    ...
    return <dialog ref={ref} onCancel={e => { e.preventDefault(); if (!ocupado) onFechar(); }} aria-labelledby="titulo-exportacao-iob" className="w-[min(95vw,640px)] rounded-xl bg-white p-6 text-slate-800 shadow-xl backdrop:bg-black/50 dark:bg-slate-800 dark:text-white">
        ...
        {cadastro && <p className="mb-4 rounded bg-amber-50 p-3 text-sm text-amber-900"><strong>Compatibilidade cadastral IOB pendente.</strong> O pacote JSON e o CSV são para conferência. Não devem ser importados na rotina de apontamentos. As pendências de histórico acompanham os dados.</p>}
