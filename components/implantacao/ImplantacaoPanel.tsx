...
function Ficha({ cadastro: c, fontePdf, onSalvar }: { cadastro: Cadastro; fontePdf: string; onSalvar: (campo: Campo, valor: string, fonte: string, motivo: string) => void }) {
    ...
    return <div className="space-y-3 rounded-xl border border-blue-300 p-4">
        ...
        <p className="text-sm">A matrícula para IOB é a original do eSocial e não pode ser renumerada nesta ficha.</p>
        <h5 className="font-semibold">Registrar informação conferida</h5>
        <div className="grid gap-3 sm:grid-cols-2">
            <label className="text-sm">Campo<select className={input} value={campo} onChange={e => { setCampo(e.target.value as Campo); setValor(''); setConfirmado(false); }}>{Object.entries(CAMPOS).filter(([k]) => k !== 'matriculaIob').map(([k, label]) => <option key={k} value={k}>{label}</option>)}</select></label>
