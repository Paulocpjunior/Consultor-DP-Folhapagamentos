...
// ─── Proxy shim: SDK substituído por fetch ao Express backend ───
// GEMINI_API_KEY mora apenas no servidor. Browser nunca vê a chave.
const ai = {
    models: {
        generateContent: async (args: any): Promise<any> => {
            ...
            return { text: data.text, ...(data.raw || {}) };
        }
    }
};
...
// Funções referenciadas por componentes do CFI que não existem mais no código.
// Mantidas como stubs pra não quebrar build/runtime. Implementar quando necessário.

export const fetchFiscalData = async (..._args: any[]): Promise<any> => {
    console.warn('[stub] fetchFiscalData chamado — implementação pendente');
    return null;
};

export const fetchComparison = async (..._args: any[]): Promise<any> => {
    console.warn('[stub] fetchComparison chamado — implementação pendente');
    return null;
};

export const fetchSimilarServices = async (..._args: any[]): Promise<any[]> => {
    console.warn('[stub] fetchSimilarServices chamado — implementação pendente');
    return [];
};

export const fetchCnaeSuggestions = async (..._args: any[]): Promise<any[]> => {
    console.warn('[stub] fetchCnaeSuggestions chamado — implementação pendente');
    return [];
};

export const fetchCnaeDescription = async (..._args: any[]): Promise<string> => {
    console.warn('[stub] fetchCnaeDescription chamado — implementação pendente');
    return '';
};

export const fetchNewsAlerts = async (..._args: any[]): Promise<any[]> => {
    console.warn('[stub] fetchNewsAlerts chamado — implementação pendente');
    return [];
};

export const fetchReformaNews = async (..._args: any[]): Promise<any[]> => {
    console.warn('[stub] fetchReformaNews chamado — implementação pendente');
    return [];
};

export const extractDocumentData = async (..._args: any[]): Promise<any> => {
    console.warn('[stub] extractDocumentData chamado — implementação pendente');
    return null;
};

export const extractPgdasDataFromPdf = async (..._args: any[]): Promise<any> => {
    console.warn('[stub] extractPgdasDataFromPdf chamado — implementação pendente');
    return null;
};
